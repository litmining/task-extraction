...
CHUNK_SIZE = 50
CSV_FILE = (
    "data/neurovault_labeled_papers/pmid_collection_statisticmaps_text_subset.csv"
)
...
def read_csv_chunk(
    csv_file: str, chunk_size: int = 10, chunk_number: int = 1
) -> Dict[str, str]:
    """
...
def process_chunk(
    ...
    for k, v in csv_data.items():
        try:
            ...
            task_info_dict = {
                key: task_info[key] if task_info[key] is not None else None
                for key in key_list
            }
            pmcid_map_file_add_index(
                index_file, k, task_file_pattern.format(chunk_number=chunk_number)
            )
            output = {
                k: {
                    "pmcid": k,
...
if __name__ == "__main__":
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        futures = {
            executor.submit(process_chunk, chunk_number): chunk_number
